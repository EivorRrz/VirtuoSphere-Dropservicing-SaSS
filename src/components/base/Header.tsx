...
const Header = () => {
    ...
    const renderLinks = () => (
        <>
            ...
            <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
                <Button className="text-base dark:text-[#ECECEC] p-0" variant="link">
                    <div onClick={handleScroll('team', 100)}>About Me</div>
                </Button>
            </motion.div>
        </>
    );
    ...
    if (isDesktop) {
        return (
            <motion.div
                ...
                className="w-full h-16 bg-white/20 dark:bg-header/20 backdrop-blur-lg top-0 fixed border-b-[1px] z-[999]"
            >
        ...
        <Sheet>
            <SheetContent side="top" className="dark:bg-header border-b-0 z-[998]">
                ...
                <SheetDescription className="flex flex-col justify-center items-center mx-auto">
                    <motion.div whileHover={{ scale: 1.1 }}>{renderLinks()}</motion.div>
                </SheetDescription>
